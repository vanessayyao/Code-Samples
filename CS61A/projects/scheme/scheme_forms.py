...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        prod = LambdaProcedure(formals, body, env)
        env.define(symbol, prod)
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest is not nil:
        if is_scheme_false(scheme_eval(expressions.first, env)): return scheme_eval(expressions.first, env)
