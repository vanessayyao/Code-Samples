...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i < num_rolls:
        roll = dice()
        if roll == 1: sow_sad = True
        total += roll
        i += 1
    if not sow_sad: return total
    else: return 1
    # END PROBLEM 1
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    score = sus_points(player_score + score)
    return score
    # END PROBLEM 4
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who == 0: 
            ...
            score0 = update(r, score0, score1, dice)
        else: 
            r = strategy1(score1, score0)
            score1 = update(r, score1, score0, dice)
        who = 1 - who
    # END PROBLEM 5
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    player, opponent = 0, 0
    dice = strategy(player, opponent)
    while player < goal:
        opponent = 0
        while opponent < goal:
            if strategy(player, opponent) != dice:
                return False
            opponent += 1
        player += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max = 0
    while i <= 10:
        new_avg = make_averaged(roll_dice, times_called)(i, dice)
        if new_avg > max: 
            max = new_avg
            num = i
        i += 1
