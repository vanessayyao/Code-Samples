...
class Place:
    ...
    def __init__(self, name: str, exit: Place | None = None):
        ...
        "*** YOUR CODE HERE ***"
        self.entrance = None
        if self.exit: (self.exit).entrance = self
        # END Problem 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        def helper(p):
            ...
            if not p: 
                return None
            if p.is_hive: return helper(p.entrance)
            if not self.position >= self.lower_bound or not self.position <= self.upper_bound:
                return helper(p.entrance)
            elif p.bees != []: 
                return random_bee(p.bees)
            else: return helper(p.entrance)
...
class TankAnt(ContainerAnt):
    ...
    def __init__(self, damage: int = 1, health: int = 2):
        super().__init__(health)
        self.damage = damage
...
class Water(Place):
    ...
    def add_insect(self, insect: Insect):
        ...
        if not insect.is_waterproof: insect.reduce_health(insect.health)
        # END Problem 10
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        super().action(gamestate)
        def helper(p):
            if p.exit == None: return
            if p.exit.ant:
                p.exit.ant.double()
                if isinstance(p.exit.ant, ContainerAnt) and p.exit.ant.ant_contained: 
                    p.exit.ant.ant_contained.double()
            return helper(p.exit)
