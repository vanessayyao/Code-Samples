...
def calc_eval(exp):
    ...
    if isinstance(exp, Link):
        ...
        operands = exp.rest.first # UPDATE THIS FOR Q2, e.g (+ 1 2), 1 and 2 are operands
        if operator == 'and': # and expressions
            return eval_and(operands)
        elif operator == 'define': # define expressions
            return eval_define(operands)
        else: # Call expressions
            return calc_apply(___________, calc_eval(exp.rest)) # UPDATE THIS FOR Q2, what is type(operator)?
