...
def balanced(m):
    ...
    torque = length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m)))
    if torque:
...
def berry_finder(t):
    ...
    for branch in branches(t):
        if not is_leaf(branch): return berry_finder(branch)
        elif label(branch) == 'berry': return True
    return False
...
def max_path_sum(t):
    ...
    max = 0
    for branch in branches(t):
        if not is_leaf(branch): max = max_path_sum(branch)
        elif label(branch) > max: max = label(branch)
    return label(t) + max
